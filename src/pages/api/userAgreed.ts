...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ResponseData>) {
    ...
    const { userId, isAgreed } = req.body as UpdatePayingStatusRequest;

    if (!userId) {
        return res.status(400).json({ message: 'User ID and valid isAgreed status are required' });
    }
    ...
    try {
        ...
        decodedToken.isAgreed = isAgreed;
